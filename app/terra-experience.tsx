...
export default function TerraExperience(){
 ...
 useEffect(()=>{const controller=new AbortController();let instance:Engine|undefined;const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;const opts={...initialOptions,motion:!reduced};
  import('@/lib/terra/engine').then(({createEarth})=>{if(controller.signal.aborted||!host.current||!markers.current)return;return createEarth(host.current,markers.current,{ready:()=>setReady(true),stage:s=>{setSettling(s==='orbit'&&previousStage.current==='ascending');previousStage.current=s;setStage(s);setExploring(false);},view:setView,arrival:id=>{setReturned(!!id);setSettling(false);},error:setError,interact:()=>setExploring(true),coordinates:(lat,lon)=>{if(coordinate.current)coordinate.current.textContent=`${formatCoordinate(lat,'N','S')}  /  ${formatCoordinate(lon,'E','W')}`;}},controller.signal);}).then(e=>{if(!e)return;if(controller.signal.aborted){e.dispose();return;}instance=e;engine.current=e;e.configure(opts);setOptions(opts);}).catch(e=>{if(e?.name!=='AbortError')setError('The star field could not open. Please try a browser with WebGL enabled, or reload to try again.');});
 ...
 async function descend(){if(!engine.current||loadingCity||flying)return;setError('');setLoadingCity(true);setSelected(null);try{await engine.current.descend();setRemembered(null);setReturned(false);}catch{}finally{setLoadingCity(false);}}
 ...
 return <main className={`experience stage-${stage} view-${view}${selected?' has-story':''}${remembered?' has-memory':''}${exploring?' is-exploring':''}${settling?' is-settling':''}${options.motion?'':' motion-paused'}`}>
  ...
  {stage==='orbit'?<DepthControls view={view} region={region} depth={options.depth} disabled={!ready||loadingCity} onView={mode=>engine.current?.view(mode)} onRegion={id=>{setRegion(id);engine.current?.region(id);}} onDepth={depth=>configure({depth})}/>:null}
