...
const initialOptions:ViewOptions={glow:1.15,shimmer:1.1,threads:.55,density:.85,borders:false,motion:true};
...
export default function TerraExperience(){
 ...
 return <main className={`experience stage-${stage}${selected?' has-story':''}${remembered?' has-memory':''}${exploring?' is-exploring':''}${settling?' is-settling':''}${options.motion?'':' motion-paused'}`}>
  ...
  <section className="opening" aria-live="polite" aria-hidden={settling}>
