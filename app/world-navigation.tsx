...
export function WorldNavigation({state,disabled,onReplay}:{state:WorldState|null;disabled:boolean;onReplay:()=>void}) {
  ...
  return <div className="world-navigation">
    ...
    {open?<section className="world-destinations" aria-label="Explore destinations">
      ...
      <p>Different places. Different worlds.</p>
      {WORLD_TARGETS.map(target=><button key={target.id} disabled={disabled} className="world-target" onClick={()=>void run({type:'flyTo',targetId:target.id})}><span>{target.label}<small>{target.id==='singapore'?'Streets, embers, everyday life':target.id==='new-york'?'The city, in motion':target.id==='challenger-deep'?'Into the ocean’s depths':target.detail}</small></span><ArrowUpRight size={16}/></button>)}
      <div className="world-layer-actions"><span>A living world</span><button onClick={()=>void run({type:'focusLayer',layer:'satellites',enabled:!state?.layers.satellites})} aria-pressed={state?.layers.satellites??true}>Orbit</button><button onClick={()=>void run({type:'focusLayer',layer:'aircraft',enabled:!state?.layers.aircraft})} aria-pressed={state?.layers.aircraft??true}>Air</button><button onClick={()=>void run({type:'focusLayer',layer:'ships',enabled:!state?.layers.ships})} aria-pressed={state?.layers.ships??true}>Sea</button></div>
