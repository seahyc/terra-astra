...
const hash=(id:number,k:number)=>fract(Math.sin(id*12.9898+k*78.233)*43758.5453);
...
export function genesisLight(progress:number){
 ...
 return {exposure:(.045+.955*ramp(.36,.67,p))*(1+flash*4.8),size:.23+.77*ramp(.36,.74,p)+flash*.48,flash,settled:ramp(.82,1,p)};
}
...
export function genesisPosition(x:number,y:number,z:number,id:number,phase:number,progress:number,out:THREE.Vector3){
 ...
 const p=Math.max(0,progress),a=hash(id,1),b=hash(id,2),c=hash(id,3),az=a*Math.PI*2,ny=b*2-1,rad=Math.sqrt(Math.max(0,1-ny*ny));
 const compression=ramp(.20,.32,p),turn=p*(12+compression*19)*(a>.8?1:-1)+phase;
 const radius=(a>.80?.13+.17*c:.008+.079*Math.pow(c,2))*(1-compression*.52);
 ...
 const nx=Math.cos(az)*rad,nz=Math.sin(az)*rad,reach=1.35+.78*c;
...
uniform float genesis;uniform float genesisEnabled;uniform float genesisExposure;uniform float genesisSize;attribute float particleId;
float genRamp(float a,float b,float p){float t=clamp((p-a)/(b-a),0.0,1.0);return t*t*(3.0-2.0*t);}
float genHash(float id,float k){return fract(sin(id*12.9898+k*78.233)*43758.5453);}
vec3 genesisPosition(vec3 home){
 if(genesisEnabled<.5||genesis>=1.0)return home;
 float p=max(0.0,genesis),a=genHash(particleId,1.0),b=genHash(particleId,2.0),c=genHash(particleId,3.0);
 ...
 float radius=(a>.80?.13+.17*c:.008+.079*pow(c,2.0))*(1.0-compression*.52),curl=.014*sin(phase*3.0+p*47.0)*(1.0-compression*.4);
 vec3 core=vec3(cos(az+turn)*rad*radius+curl*ny,ny*radius+sin(turn*.7+phase)*radius*.18,sin(az+turn)*rad*radius-curl*rad);
 vec3 n=vec3(cos(az)*rad,ny,sin(az)*rad),burst=n*(1.35+.78*c);
