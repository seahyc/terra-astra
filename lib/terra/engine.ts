...
import { GENESIS_DURATION, genesisGLSL, genesisLight, genesisState } from './genesis';
...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal):Promise<Engine>{
 ...
 const g=geometry(p);const ids=new Float32Array(count);for(let i=0;i<count;i++)ids[i]=particleOffset+i+1;particleOffset+=count;g.setAttribute('particleId',new THREE.BufferAttribute(ids,1));g.setAttribute('brightness',new THREE.BufferAttribute(b,1));g.setAttribute('starSize',new THREE.BufferAttribute(s,1));g.setAttribute('phase',new THREE.BufferAttribute(ph,1));g.setAttribute('revealAt',new THREE.BufferAttribute(new Float32Array(count),1));
 ...
 reliefLand=spatialCloud(data[5],'#8f9598');reliefOcean=spatialCloud(data[6],'#536c77');body=spatialCloud(data[7],'#a79a83');interior=spatialCloud(data[8],'#b4a17f');haze=spatialCloud(data[9],'#887f75');halo=spatialCloud(data[10],'#c7d0d6');haze.material.uniforms.soft.value=1;
 ...
 function setRegion(id:StudyRegion){if(stage!=='orbit'||flight||morph||genesisProgress<1||openProgress>0)return;region=id;const r=studyRegions[id];targetLat=r.lat;targetLon=lon+wrap(r.lon-lon);cutFacing.copy(geo(r.lon,r.lat));cutNormal.copy(geo(r.lon+90,0));lastInteraction=performance.now();stillFrames=0;callbacks.interact();}
 ...
 async function executeCommand(cmd:WorldCommand):Promise<WorldCommandResult>{
  ...
  const target=cmd.type==='flyTo'||cmd.type==='highlightTarget'?WORLD_TARGETS.find(t=>t.id===cmd.targetId):WORLD_TARGETS.find(t=>t.id===targetId);
  if((cmd.type==='flyTo'||cmd.type==='highlightTarget')&&!target)return {ok:false,command:cmd,reason:'Unknown target'};
  if(cmd.type==='highlightTarget'){targetId=cmd.targetId;stillFrames=0;notifyWorld();return {ok:true,command:cmd};}
  ...
  try{
   if(target?.id==='singapore'&&(tier==='city'||tier==='street'))await ensureCity();
   if(disposed)return {ok:false,command:cmd,reason:'World is unavailable'};
   clearStory();remembering=false;remembered=null;arrivalSent=true;settledAt=null;targetId=cmd.type==='resetView'?null:target?.id??null;scaleTier=tier;
   const destinationAlt=tier==='planet'?homeAltitude():tier==='region'?.20:tier==='city'?.0018:.00075;
   fly(target?.lat??19,target?.lon??95,destinationAlt,tier==='city'||tier==='street'?'city':'orbit');notifyWorld();
 ...
 function zoom(f:number){if(flight||morph||genesisProgress<1)return;callbacks.interact();lastInteraction=performance.now();stillFrames=0;targetAlt=clamp(targetAlt*f,stage==='city'?.00055:openProgress>0?openAltitude()*.78:viewMode==='oblique'?.32:.42,stage==='city'?.004:homeAltitude()*(openProgress>0?2.5:1.8));}
 ...
 function frame(now:number){if(disposed||graphicsLost||document.hidden)return;if(!options.motion&&stillFrames>2&&!flight&&!morph&&genesisProgress>=1&&personalSettledAt===null){raf=requestAnimationFrame(frame);return;}if(fallback&&now-lastTime<50){raf=requestAnimationFrame(frame);return;}const dt=Math.min((now-lastTime)/1000,.06);lastTime=now;time+=dt;frameCount++;stillFrames++;
 ...
 if(!options.motion||frameCount%2===0){placeLabel('singapore',1.30,103.85,stage==='orbit'&&viewMode==='globe'&&!morph&&genesisProgress>=1&&openProgress===0&&!personalPlaces);for(const s of stories)placeLabel(s.id,s.lat,s.lon,stage==='city'&&!selected);for(let i=0;i<3;i++){const p=selected?.places[i];placeLabel('place-'+i,p?.lat||0,p?.lon||0,stage==='city'&&!!p&&reveal.stars>i*.25+.15);}}
