...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal):Promise<Engine>{
 ...
 const urbanViews=new Map<string,UrbanView>();
 ...
 const destroy=()=>{if(disposed)return;disposed=true;rejectEvidence('Earth renderer disposed.');evidenceOverlay.dispose();for(const resolve of idleWaiters.splice(0))resolve(false);cancelAnimationFrame(raf);observer.disconnect();host.removeEventListener('pointerdown',down);host.removeEventListener('pointermove',move);host.removeEventListener('pointerup',up);host.removeEventListener('pointercancel',cancel);host.removeEventListener('wheel',wheel);renderer.domElement.removeEventListener('keydown',key);renderer.domElement.removeEventListener('webglcontextlost',contextLost);document.removeEventListener('visibilitychange',visibility);for(const g of geometries)g.dispose();for(const m of materials)m.dispose();renderer.dispose();renderer.domElement.remove();};
 ...
 function createUrbanView(id:string,roads:Float32Array,feather:(lon:number,lat:number)=>number){
  const model=createUrbanActivity(roads,{trafficCount:host.clientWidth<700?440:760,activityCount:host.clientWidth<700?320:600,seed:id==='singapore'?260913:260914});
...
if(!isBusy())for(const resolve of idleWaiters.splice(0))resolve(true);
 ...
 return {presentEvidenceScene,clearEvidenceScene,replayGenesis,skipGenesis,command,worldState,transform:setTransformation,personal:setPersonal,descend:async()=>{if(flight||morph||genesisProgress<1||openProgress>0)return;try{await ensureCity();if(disposed)return;personalSettledAt=null;personalArrivalSent=false;clearStory();remembered=null;remembering=false;settledAt=null;arrivalSent=false;targetId='singapore';scaleTier='city';fly(1.2965,103.851,.0018,'city');notifyWorld();}catch(e){if(!signal.aborted)callbacks.error('Singapore could not load. Your Earth is still here; try entering again.');throw e;}},orbit:()=>{if(flight||morph||stage!=='city')return;clearStory();remembering=!!remembered;settledAt=null;arrivalSent=false;targetId=null;scaleTier='planet';fly(19,95,homeAltitude(),'orbit');notifyWorld();},zoom,rotate,select,view:setView,region:setRegion,configure:(o)=>{options=o;if(flight&&!o.motion)flight.duration=0;if(evidenceJourney&&!o.motion)evidenceJourney.duration=0;lastInteraction=performance.now();stillFrames=0;},storyInset:setStoryInset,dispose:destroy};
}
