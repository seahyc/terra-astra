...
import { makeAngkorParticles } from './sculptures/angkor-particles';
import { createEarthEvidenceOverlay } from './presentation/earth-overlay';
import type { EvidenceScene, RendererReady } from './evidence/types';
...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal):Promise<Engine>{
 ...
 const idleWaiters:Array<(active:boolean)=>void>=[];let commandChain:Promise<unknown>=Promise.resolve(),commandSequence=0;let warmupTimer:ReturnType<typeof setTimeout>|null=null,warmupScheduled=false;
 ...
 let evidenceScene:EvidenceScene|null=null;
 let sculptureCloud:Cloud|null=null;
 let sculptureStarted=0;
 let evidenceJourney:{start:number;duration:number;fromLat:number;fromLon:number;fromAlt:number;wideAlt:number}|null=null;
 let evidencePending:{revision:number;resolve:(value:RendererReady)=>void;reject:(error:Error)=>void;cleanup:()=>void;started:number;frames:number}|null=null;
 const evidenceOverlay=createEarthEvidenceOverlay(host,(p,h)=>geo(p[0],p[1],h===undefined?1.00004:1+(reliefRadius(h)-1)*depth+.00008));
 const rejectEvidence=(message:string)=>{if(evidencePending){const pending=evidencePending;evidencePending=null;pending.cleanup();pending.reject(new Error(message));}};
 function clearEvidenceScene(){rejectEvidence('Presentation cleared.');evidenceScene=null;evidenceJourney=null;evidenceOverlay.clear();if(sculptureCloud)sculptureCloud.material.uniforms.opacity.value=0;stillFrames=0;notifyWorld();}
 function frameEvidenceCamera(next:EvidenceScene){
   if(next.sculpture){targetAlt=.00068;return;}
   const b=next.bounds;
   const usableWidth=Math.max(180,host.clientWidth-(host.clientWidth>=700?430:35));
   const usableHeight=Math.max(160,host.clientHeight-(host.clientWidth<700?host.clientHeight*.48+90:150));
   const latSpan=Math.max(.035,b.north-b.south)*R;
   const lonSpan=Math.max(.035,b.east-b.west)*R*Math.cos(targetLat*R);
   targetAlt=Math.max(next.view==='profile'?.22:.0012,Math.max(latSpan*host.clientHeight/usableHeight,lonSpan*host.clientHeight/usableWidth)/(2*Math.tan(21*R))*1.65);
 }
 async function presentEvidenceScene(next:EvidenceScene,presentationSignal:AbortSignal):Promise<RendererReady>{
   if(disposed||graphicsLost||presentationSignal.aborted)throw new Error('Presentation cancelled.');
   if(evidenceScene&&next.revision<=evidenceScene.revision)throw new Error('Stale scene revision.');
   rejectEvidence('Presentation superseded.');evidenceJourney=null;
   if(!await waitIdle())throw new Error('World is unavailable.');
   if(openProgress>0){setTransformation(false);if(!await waitIdle())throw new Error('World is unavailable.');}
   if(next.view!=='profile')await ensureCity();
   if(disposed||graphicsLost||presentationSignal.aborted)throw new Error('Presentation cancelled.');
   const b=next.bounds;
   if(![b.west,b.east,b.south,b.north].every(Number.isFinite)||b.west>b.east||b.south>b.north)throw new Error('Invalid geographic bounds.');
   flight=null;clearStory();remembered=null;remembering=false;settledAt=null;arrivalSent=false;targetId=null;scaleTier='planet';highlight(null);setStage('orbit');
   viewMode=next.perspective??(next.view==='profile'?'oblique':'globe');callbacks.view?.(viewMode);
   targetLat=(b.south+b.north)/2;targetLon=lon+wrap((b.west+b.east)/2-lon);targetTilt=next.sculpture?62:viewMode==='oblique'?55:0;
   frameEvidenceCamera(next);
   if(sculptureCloud)sculptureCloud.material.uniforms.opacity.value=0;
   if(next.sculpture==='angkor-wat'){
     if(!sculptureCloud){
       const particles=makeAngkorParticles(), center=geo(103.867,13.4125,1.000003), up=geo(103.867,13.4125), east=geo(103.867+90,0), north=geo(103.867,13.4125+90), scale=.00014;
       for(let i=0;i<particles.length;i+=6){const p=center.clone().addScaledVector(east,particles[i]*scale).addScaledVector(up,particles[i+1]*scale).addScaledVector(north,particles[i+2]*scale);particles.set(p.toArray(),i);particles[i+4]*=.32;particles[i+3]*=.65;}
       sculptureCloud=cloud(particles,'#fff0ca');sculptureCloud.points.userData.sampleBudget=14000;sculptureCloud.points.userData.fallbackExposure=.65;
       const reveal=sculptureCloud.points.geometry.getAttribute('revealAt');for(let i=0;i<reveal.count;i++)reveal.setX(i,(i/reveal.count)*.65);
     }
     sculptureStarted=performance.now();
   }
   const angularTravel=Math.hypot(targetLat-lat,wrap(targetLon-lon)*Math.cos(targetLat*R));
   evidenceJourney=options.motion?{start:performance.now(),duration:angularTravel>4?4200:2400,fromLat:lat,fromLon:lon,fromAlt:alt,wideAlt:Math.max(alt,targetAlt,angularTravel>4?.48:targetAlt*1.5)}:null;
   evidenceScene=next;evidenceOverlay.set(next);lastInteraction=performance.now();stillFrames=0;
   return new Promise((resolve,reject)=>{const aborted=()=>rejectEvidence('Presentation cancelled.');presentationSignal.addEventListener('abort',aborted,{once:true});evidencePending={revision:next.revision,resolve,reject,cleanup:()=>presentationSignal.removeEventListener('abort',aborted),started:performance.now(),frames:0};notifyWorld();});
 }
 ...
 function resize(){stillFrames=0;const w=host.clientWidth,h=host.clientHeight;if(!w||!h)return;const nextHomeAltitude=homeAltitude();if(!evidenceScene&&stage==='orbit'&&scaleTier==='planet'&&viewMode==='globe'&&!flight&&!morph){const scale=nextHomeAltitude/previousHomeAltitude;alt*=scale;targetAlt*=scale;}if(flight?.end==='orbit'&&scaleTier==='planet')flight.toAlt=nextHomeAltitude;previousHomeAltitude=nextHomeAltitude;if(evidenceScene)frameEvidenceCamera(evidenceScene);renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();if(selected&&storyInset)setStoryInset(storyInset);}
 const observer=new ResizeObserver(resize);observer.observe(host);resize();
 const destroy=()=>{if(disposed)return;disposed=true;if(warmupTimer!==null)clearTimeout(warmupTimer);rejectEvidence('Earth renderer disposed.');evidenceOverlay.dispose();for(const resolve of idleWaiters.splice(0))resolve(false);cancelAnimationFrame(raf);observer.disconnect();host.removeEventListener('pointerdown',down);host.removeEventListener('pointermove',move);host.removeEventListener('pointerup',up);host.removeEventListener('pointercancel',cancel);host.removeEventListener('wheel',wheel);renderer.domElement.removeEventListener('keydown',key);renderer.domElement.removeEventListener('webglcontextlost',contextLost);document.removeEventListener('visibilitychange',visibility);for(const g of geometries)g.dispose();for(const m of materials)m.dispose();renderer.dispose();renderer.domElement.remove();};
 const contextLost=(e:Event)=>{e.preventDefault();graphicsLost=true;rejectEvidence('Graphics became unavailable.');for(const resolve of idleWaiters.splice(0))resolve(false);notifyWorld();callbacks.error('Graphics became unavailable. Restart the journey to gather the stars again.',true);cancelAnimationFrame(raf);};renderer.domElement.addEventListener('webglcontextlost',contextLost);
 try{
 ...
 function select(id:string|null){if(stage!=='city'||targetId==='new-york'||flight||morph)return;if(!id){clearStory();return;}const s=stories.find(x=>x.id===id);if(!s)return;clearStory();selected=s;remembered=s;storyStarted=performance.now();targetLat=(Math.min(...s.places.map(p=>p.lat))+Math.max(...s.places.map(p=>p.lat)))/2;targetLon=(Math.min(...s.places.map(p=>p.lon))+Math.max(...s.places.map(p=>p.lon)))/2;targetAlt=host.clientWidth<700?.0019:.00125;if(storyInset)setStoryInset(storyInset);lastInteraction=performance.now();stillFrames=0;const pts:number[]=[],links:number[]=[];for(const p of s.places)pts.push(...geo(p.lon,p.lat,1.00003).toArray(),1.4,4.1,6.15);
 ...
 function setTransformation(open:boolean){
   ...
   clearEvidenceScene();const now=performance.now();openTarget=open;personalArrivalSent=false;personalSettledAt=null;lastInteraction=now;stillFrames=0;
 ...
 function isBusy(){return genesisProgress<1||!!flight||!!morph||loadingDetail||!!evidencePending||!!evidenceJourney;}
 ...
 async function executeCommand(cmd:WorldCommand,queuedAt:number,sequence:number):Promise<WorldCommandResult>{
  ...
  try{
   ...
   clearEvidenceScene();clearStory();remembering=false;remembered=null;arrivalSent=true;settledAt=null;targetId=cmd.type==='resetView'?null:target?.id??null;scaleTier=tier;highlight(targetId);
   ...
   await detail;if(!await waitIdle())return {ok:false,command:cmd,reason:'World is unavailable'};host.dataset.worldCommandSettledAt=performance.now().toFixed(1);host.dataset.worldCommandDurationMs=(performance.now()-startedAt).toFixed(1);host.dataset.worldCommandStatus='settled';notifyWorld();return {ok:true,command:cmd};
 ...
 function command(input:WorldCommand):Promise<WorldCommandResult>{const cmd=validateWorldCommand(input);if(!cmd)return Promise.resolve({ok:false,command:input,reason:'Invalid world command'});const queuedAt=performance.now(),sequence=++commandSequence;host.dataset.worldCommandQueuedAt=queuedAt.toFixed(1);host.dataset.worldCommandStatus='queued';const result=commandChain.then(()=>executeCommand(cmd,queuedAt,sequence));commandChain=result.catch(()=>undefined);return result;}
 const pointer=new Map<number,{x:number;y:number}>();let dragged=false,initial={x:0,y:0},pinch=0;
 function rotate(dx:number,dy:number){if(flight||evidenceJourney||morph||genesisProgress<1)return;callbacks.interact();lastInteraction=performance.now();stillFrames=0;const scale=stage==='city'?targetAlt*2*Math.tan(21*R)/(R*host.clientHeight):.18*Math.min(1,targetAlt/1.4);targetLon-=dx*scale;targetLat=clamp(targetLat+dy*scale,-80,80);if(stage==='city'){const ny=targetId==='new-york';targetLon=clamp(targetLon,ny?-74.015:103.810,ny?-73.975:103.890);targetLat=clamp(targetLat,ny?40.707:1.272,ny?40.737:1.330);}}
 function zoom(f:number){if(flight||evidenceJourney||morph||genesisProgress<1)return;callbacks.interact();lastInteraction=performance.now();stillFrames=0;targetAlt=clamp(targetAlt*f,stage==='city'?.00055:openProgress>0?openAltitude()*.78:viewMode==='oblique'?.32:scaleTier==='region'?.10:.42,stage==='city'?.004:homeAltitude()*(openProgress>0?2.5:1.8));}
 function down(e:PointerEvent){if(flight||evidenceJourney||morph||genesisProgress<1||!(e.target instanceof HTMLCanvasElement))return;renderer.domElement.focus({preventScroll:true});pointer.set(e.pointerId,{x:e.clientX,y:e.clientY});host.setPointerCapture(e.pointerId);initial={x:e.clientX,y:e.clientY};dragged=false;lastInteraction=performance.now();stillFrames=0;if(pointer.size===2){const p=[...pointer.values()];pinch=Math.hypot(p[0].x-p[1].x,p[0].y-p[1].y);}}
 ...
 north.copy(geo(lon,lat+90));camera.position.copy(geo(lon,lat)).addScaledVector(geo(lon,lat),alt*Math.cos(tilt*R)).addScaledVector(evidenceScene?.sculpture?north.clone().multiplyScalar(.866).addScaledVector(geo(lon+90,0),.5):north,-alt*Math.sin(tilt*R));camera.up.copy(evidenceScene?.sculpture&&tilt>1?geo(lon,lat):north);camera.lookAt(geo(lon,lat).multiplyScalar(born.settled));const w=host.clientWidth,h=host.clientHeight,mobile=w<700;const offsetY=evidenceScene?(mobile?h*(evidenceScene.sculpture?.055:.19):0):mobile?(selected?(storyInset?Math.max(0,(storyInset-70)/2):h*.225):openProgress>0?h*.25:personalPlaces&&stage==='orbit'?-h*.09:viewMode==='cutaway'?h*.11:-h*.035):0;smoothCameraOffset.lerp(new THREE.Vector2(mobile?0:evidenceScene?w*.15:-w*.17*born.settled,offsetY*born.settled),frameCount===1||!options.motion?1:1-Math.exp(-dt*4));camera.setViewOffset(w,h,smoothCameraOffset.x,smoothCameraOffset.y,w,h);camera.near=Math.max(.0000004,alt*.002);camera.far=65;camera.updateProjectionMatrix();camera.updateMatrixWorld();
 ...
 if(sculptureCloud){const shown=evidenceScene?.sculpture==='angkor-wat';sculptureCloud.material.uniforms.opacity.value=shown?1:0;sculptureCloud.material.uniforms.reveal.value=options.motion?Math.min(1,(now-sculptureStarted)/2800):1;}
 if(personalCloud){
 ...
 if(!options.motion||frameCount%2===0){placeLabel('singapore',1.30,103.85,!evidenceScene&&stage==='orbit'&&viewMode==='globe'&&!morph&&genesisProgress>=1&&openProgress===0&&!personalPlaces);for(const s of stories)placeLabel(s.id,s.lat,s.lon,stage==='city'&&targetId!=='new-york'&&!selected);for(let i=0;i<3;i++){const p=selected?.places[i];placeLabel('place-'+i,p?.lat||0,p?.lon||0,stage==='city'&&!!p&&reveal.stars>i*.25+.15);}}
 for(let i=0;i<3;i++)placePersonalLabel(i,!!personalPlaces&&stage==='orbit'&&openProgress>.72&&!morph);
 renderer.render(scene,camera);evidenceOverlay.update(camera);
 if(evidencePending){evidencePending.frames++;const settled=!evidenceJourney&&Math.abs(lat-targetLat)<.00001&&Math.abs(lon-targetLon)<.00001&&Math.abs(alt-targetAlt)<.000001&&Math.abs(tilt-targetTilt)<.01;
 if(settled&&evidencePending.frames>=2){const pending=evidencePending;evidencePending=null;pending.cleanup();pending.resolve({revision:pending.revision,ready:true,renderer:fallback?'canvas':'webgl'});notifyWorld();}
 else if(now-evidencePending.started>12000)rejectEvidence('Earth camera did not settle in time.');}
if(!isBusy())for(const resolve of idleWaiters.splice(0))resolve(true);
 ...
 diagnosticFrames++;if(now-diagnosticStart>=1000){host.dataset.renderFps=(diagnosticFrames*1000/(now-diagnosticStart)).toFixed(1);host.dataset.renderFrames=String(frameCount);host.dataset.renderFrameMs=((now-diagnosticStart)/diagnosticFrames).toFixed(1);diagnosticStart=now;diagnosticFrames=0;}if(!options.motion||now-lastCoordinate>500){callbacks.coordinates(lat,wrap(lon));lastCoordinate=now;}
 ...
 return {presentEvidenceScene,clearEvidenceScene,replayGenesis,skipGenesis,command,worldState,transform:setTransformation,personal:setPersonal,descend:async()=>{if(flight||morph||genesisProgress<1||openProgress>0)return;try{personalSettledAt=null;personalArrivalSent=false;clearStory();remembered=null;remembering=false;settledAt=null;arrivalSent=false;targetId='singapore';scaleTier='city';loadingDetail=true;fly(1.2965,103.851,.0018,'city');notifyWorld();try{await ensureCity();}finally{loadingDetail=false;stillFrames=0;}if(disposed)return;}catch(e){if(!signal.aborted)callbacks.error('Singapore could not load. Your Earth is still here; try entering again.');throw e;}},orbit:()=>{if(flight||morph||stage!=='city')return;clearStory();remembering=!!remembered;settledAt=null;arrivalSent=false;targetId=null;scaleTier='planet';fly(19,95,homeAltitude(),'orbit');notifyWorld();},zoom,rotate,select,view:setView,region:setRegion,configure:(o)=>{options=o;if(flight&&!o.motion)flight.duration=0;if(evidenceJourney&&!o.motion)evidenceJourney.duration=0;lastInteraction=performance.now();stillFrames=0;},storyInset:setStoryInset,dispose:destroy};
}
