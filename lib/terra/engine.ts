...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal):Promise<Engine>{
 ...
 const destroy=()=>{if(disposed)return;disposed=true;for(const resolve of idleWaiters.splice(0))resolve(false);cancelAnimationFrame(raf);observer.disconnect();host.removeEventListener('pointerdown',down);host.removeEventListener('pointermove',move);host.removeEventListener('pointerup',up);host.removeEventListener('pointercancel',cancel);host.removeEventListener('wheel',wheel);renderer.domElement.removeEventListener('keydown',key);renderer.domElement.removeEventListener('webglcontextlost',contextLost);document.removeEventListener('visibilitychange',visibility);for(const g of geometries)g.dispose();for(const m of materials)m.dispose();renderer.dispose();renderer.domElement.remove();};
 ...
 function resetView(){viewMode='globe';targetTilt=0;perspectiveSettling=false;callbacks.view?.('globe');}
 ...
 function worldState():WorldState{return {targetId,...(dynamicLocation&&targetId==='location'?{location:{...dynamicLocation}}:{}),tier:scaleTier,perspective:perspective(),busy:isBusy(),genesis:genesisState(genesisProgress),layers:{...layerFlags}};}
 ...
 async function executeCommand(cmd:WorldCommand):Promise<WorldCommandResult>{
  ...
  if(cmd.type==='setPerspective'){
   ...
   if(stage==='city'&&mode==='cutaway'){
    ...
    scaleTier='planet';fly(current.lat,current.lon,homeAltitude(),'orbit');notifyWorld();if(!await waitIdle())return {ok:false,command:cmd,reason:'World is unavailable'};
   }
  ...
  try{
   ...
   clearStory();remembering=false;remembered=null;arrivalSent=true;settledAt=null;dynamicLocation=cmd.type==='flyToLocation'?{name:cmd.name,latitude:cmd.latitude,longitude:cmd.longitude,span:cmd.span}:target?.id==='location'?dynamicLocation:null;targetId=cmd.type==='resetView'?null:target?.id??null;scaleTier=tier;highlight(targetId);
 ...
 return {replayGenesis,skipGenesis,command,worldState,transform:setTransformation,personal:setPersonal,descend:async()=>{if(flight||morph||genesisProgress<1||openProgress>0)return;try{await ensureCity();if(disposed)return;personalSettledAt=null;personalArrivalSent=false;clearStory();remembered=null;remembering=false;settledAt=null;arrivalSent=false;targetId='singapore';dynamicLocation=null;scaleTier='city';fly(1.2965,103.851,.0018,'city');notifyWorld();}catch(e){if(!signal.aborted)callbacks.error('Singapore could not load. Your Earth is still here; try entering again.');throw e;}},orbit:()=>{if(flight||morph||stage!=='city')return;clearStory();remembering=!!remembered;settledAt=null;arrivalSent=false;targetId=null;dynamicLocation=null;scaleTier='planet';fly(19,95,homeAltitude(),'orbit');notifyWorld();},zoom,rotate,select,view:setView,region:setRegion,configure:(o)=>{options=o;if(flight&&!o.motion)flight.duration=0;lastInteraction=performance.now();stillFrames=0;},storyInset:setStoryInset,dispose:destroy};
}
