...
export function createAnswerRouter({ worldAnswerSchema, runProbe, fastModel = 'gpt-5.6-luna', answerModel = 'gpt-5.6-terra', fetchImpl = fetch }) {
  return async function answerQuestion({ apiKey, question, inventory, signal, onOpening = () => {} }) {
    ...
    if (decision.route === 'quick' && !decision.needsWeb) {
      ...
      return { world, sources: usedSuppliedSources(world, suppliedSources), kind: 'direct_answer', measured: { output: world.explanation, model: fastModel, route: 'quick', subagent_count: 0, elapsed_ms: Date.now() - started } };
    }
    ...
    return { world, sources: mergedSources(sources, usedSuppliedSources(world, suppliedSources)), kind: 'agents_terrain_answer', measured: { output: world.explanation, model: 'gpt-6-astra', route: 'research', subagent_count: report.subagent_ids.length, elapsed_ms: Date.now() - started } };
  };
}
