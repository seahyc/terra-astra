...
export function createTerraHttpHandler({ apiKey, authorize, planQuestion, worldAnswerSchema, inventory, runProbe, streamOpening, imageService, modelService, libraryService, answerQuestion, fetchImpl = fetch }) {
  ...
  return async function handle(request) {
    ...
    try {
      ...
      if (request.method === 'GET' && path === '/models') return json({models: await libraryService?.list() ?? []});
      ...
      if (!['/session', '/answer', '/image', '/model', '/library-model', '/agents/cancel'].includes(path)) return json({ error: 'Not found.' }, 404);
      if (path === '/agents/cancel') { active.get(user)?.abort(); return json({ cancellation_requested: active.has(user) }); }
      const busyAnswer = () => path === '/answer' && (active.has(user) || active.size >= 4);
      const conflict = () => json({ error: 'An answer is still being prepared. Please retry shortly.' }, 409);
      // A retry that cannot start work must not consume the inference budget.
      if (busyAnswer()) return conflict();
      const now = Date.now();
      ...
      if (budget.count >= 12 || requests.size >= 1000 && !requests.has(user)) return Response.json({ error: 'Please wait a minute before asking again.' }, { status: 429, headers: { ...headers, 'Retry-After': String(Math.max(1, Math.ceil((budget.until - now) / 1000))) } });
      budget.count++; requests.set(user, budget);
      const input = await body(request, ['/image','/model','/library-model'].includes(path) ? 16384 : 65536);
      if (path === '/library-model') return libraryService ? json(await libraryService.generate({apiKey,input,signal:request.signal})) : json({error:'Model library unavailable.'},503);
      ...
      if (path === '/image') return json(await imageService.generate({ apiKey, input, signal: request.signal }));
      if (path === '/session') {
        ...
        const response = await fetchImpl('https://api.openai.com/v1/live/sessions', {
          ...
          body: JSON.stringify({ session: { model: 'gpt-live-1', audio: {output: {voice: 'ripple'}}, delegation: { type: 'client' }, instructions: 'You are Astra, the concise voice of an interactive Earth. Speak with a playful, lightly cheeky delivery and occasional dry humour. Keep answers concise and useful; skip jokes for serious or sensitive topics. Acknowledge a new question briefly and neutrally, then delegate to the client for grounded details, navigation and models concurrently. Do not give a speculative answer before the grounded result arrives. When the client supplies a complete grounded paragraph to speak, read it verbatim in full exactly once, including every sentence, without summarizing it or adding another introduction. Let the user interrupt at any time. Start reading the supplied paragraph immediately without waiting for model construction or camera movement. For facts or measurements that are not supplied, explain what needs checking instead of guessing. Never claim a model is visible until the client confirms it. Use aerial, horizon or cutaway when suitable through the shared navigation interface. Never invent camera success or measurements. Let users interrupt. Avoid repetitive caveats; mention uncertainty when it matters.' }, transport: { type: 'webrtc', sdp: input.sdp } }),
        });
      ...
      if (busyAnswer()) { budget.count--; return conflict(); }
      ...
      const status = Number.isInteger(error?.status) ? error.status : error?.name === 'TimeoutError' ? 504 : error?.name === 'AbortError' ? 499 : 500;
