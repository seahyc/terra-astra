...
- `Engine.personal(places: PersonalPlaces | null)` creates exactly three warm stars plus a three-edge connection. Separated Astra endpoints derive deterministically from the three great-circle distances, with bounded ratio compression for readable close/collinear choices and coordinate-derived depth. Different triples form different personal geometry. Reform endpoints are the visitor's actual geographic coordinates, artistically lifted 8.5% above the Earth radius. Geographic threads use spherical interpolation; antipodal interpolation has a deterministic fallback. Far-side personal light remains perceptible through the luminous body rather than being depth-occluded.
